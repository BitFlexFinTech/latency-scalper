...
export function SSHTerminalModal({ instance, onClose }: SSHTerminalModalProps) {
  ...
  useEffect(() => {
    ...
    const initTerminal = async () => {
      ...
      try {
        ...
        // Resize handler for older xterm package without FitAddon
        const resizeTerminal = () => {
          if (terminalRef.current) {
            const geometry = term.proposeGeometry();
            if (geometry) {
  ...
  useEffect(() => {
    if (xtermRef.current?.term && terminalRef.current) {
      setTimeout(() => {
        const geometry = xtermRef.current.term.proposeGeometry();
        if (geometry) {
          xtermRef.current.term.resize(geometry.cols, geometry.rows);
        }
      }, 100);
    }
  }, [isFullscreen]);
