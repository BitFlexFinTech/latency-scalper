...
export const ExchangePulsePanel = forwardRef<HTMLDivElement, ExchangePulsePanelProps>(({ compact = false }, ref) => {
  ...
  const fetchPulses = useCallback(async () => {
    try {
      // SSOT FIX: Get connected exchanges from backend API instead of direct Supabase query
      ...
      // Build pulses from backend API data (REAL DATA ONLY from latency_logs)
      // Backend API already provides latency from latency_logs table
      const pulses: ExchangePulse[] = connectedExchanges.map(exchange => {
        const venue = exchange.name.toLowerCase().replace(/[.\s-]/g, '');
        
        // Use latency from backend API (which comes from latency_logs - REAL DATA)
        const latencyMs = exchange.latency || null;
        
        // Determine status based on latency (REAL DATA)
        let status: 'healthy' | 'jitter' | 'error' = 'healthy';
        if (latencyMs === null) {
          status = 'error'; // No latency data
        ...
        // Find corresponding timestamp from latency samples in backend response
        const latencySample = systemStatus.latency.samples?.find(s => 
          s.venue.toLowerCase() === venue
        );
        
        return {
          id: `pulse-${venue}`,
          exchange_name: exchange.name.toUpperCase(),
          status,
          latency_ms: latencyMs || 0,
          source: 'vps' as const,
          last_check: latencySample?.ts || null
        } as ExchangePulse;
      });
      
      const validPulses = pulses;
      
      // Sort alphabetically and limit
      const sorted = validPulses
