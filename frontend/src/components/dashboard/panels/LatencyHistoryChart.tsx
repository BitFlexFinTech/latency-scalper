...
export function LatencyHistoryChart() {
  ...
  const fetchHistory = async () => {
    // CRITICAL FIX: New bot uses 'venue' and 'ts' columns, not 'exchange_name' and 'recorded_at'
    const normalizedVenue = selectedExchange.toLowerCase().replace(/[.\s-]/g, '');
    
    const { data: history, error } = await supabase
      .from('latency_logs')
      .select('latency_ms, ts, venue')
      .eq('venue', normalizedVenue)
      .order('ts', { ascending: true })
      .limit(50);

    if (error) {
      console.error('[LatencyHistoryChart] Error fetching latency:', error);
      setData([]);
