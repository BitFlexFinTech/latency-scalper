...
interface TradeRecord {
  ...
  quantity: number; // Maps to position_size_usd from trade_logs
  pnl: number | null; // Maps to net_profit_usd or profit_usd from trade_logs
  status: string | null;
  ai_reasoning: string | null;
  execution_latency_ms: number | null; // Maps to duration_ms from trade_logs
  created_at: string; // Maps to entry_time from trade_logs
  closed_at: string | null; // Maps to exit_time from trade_logs
}
...
export function TradeReplayPanel() {
  ...
  // CRITICAL FIX: Fetch from trade_logs (NEW BOT) - NOT trade_logs (OLD BOT)
  const fetchTrades = async () => {
    ...
    if (!error && data) {
      // Map to TradeRecord interface
      const mappedTrades: TradeRecord[] = data.map(t => ({
        id: t.id,
        exchange: t.exchange,
        symbol: t.symbol,
        side: t.side,
        ...
        quantity: t.position_size_usd, // Map position_size_usd to quantity
        pnl: t.net_profit_usd !== null ? t.net_profit_usd : t.profit_usd,
        status: t.status,
        ai_reasoning: null,
        execution_latency_ms: t.duration_ms,
        created_at: t.entry_time || new Date().toISOString(),
  ...
  useEffect(() => {
    ...
    // CRITICAL FIX: Subscribe to trade_logs (NEW BOT) - NOT trade_logs (OLD BOT)
