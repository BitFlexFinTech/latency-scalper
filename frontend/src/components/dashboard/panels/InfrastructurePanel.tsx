  ...
  const fetchPulses = useCallback(async () => {
    try {
      ...
      const pulses: ExchangePulse[] = connectedExchanges.map(exchange => {
        ...
        // Use latency from backend API (which comes from latency_logs)
      ...
      setPulses(pulses);
      console.log('[InfrastructurePanel] Updated pulses from backend API (REAL DATA):', pulses);
    } catch (err) {
      console.error('[InfrastructurePanel] Error fetching pulses from backend API:', err);
      setPulses([]);
    }
  }, []);
