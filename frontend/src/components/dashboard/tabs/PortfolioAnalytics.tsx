    ...
    const fetchAnalytics = async () => {
      try {
        ...
        const { data: trades, error: tradesError } = await supabase
          .from('trade_logs')
          .select('profit_usd, net_profit_usd, status')
          .eq('status', 'closed');
        ...
        if (!tradesError && trades && trades.length > 0) {
          ...
          const tradesWithProfit = trades.map(t => ({
            profit: t.net_profit_usd !== null ? t.net_profit_usd : t.profit_usd
          }));
          
          const winningTrades = tradesWithProfit.filter(t => (t.profit ?? 0) > 0).length;
          const winRate = (winningTrades / tradesWithProfit.length) * 100;

          const totalProfit = tradesWithProfit.filter(t => (t.profit ?? 0) > 0).reduce((sum, t) => sum + (t.profit ?? 0), 0);
          const totalLoss = Math.abs(tradesWithProfit.filter(t => (t.profit ?? 0) < 0).reduce((sum, t) => sum + (t.profit ?? 0), 0));
          ...
          for (const t of tradesWithProfit) {
