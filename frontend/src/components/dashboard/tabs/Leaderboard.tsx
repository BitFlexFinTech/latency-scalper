  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // CRITICAL FIX: Fetch best trading days from trade_logs (NEW BOT) - NOT trade_logs (OLD BOT)
        const { data: journalData } = await supabase
          .from('trade_logs')
          .select('entry_time, profit_usd, net_profit_usd')
          .not('profit_usd', 'is', null)
          .eq('status', 'closed');

        if (journalData && journalData.length > 0) {
          const dailyStats = journalData.reduce((acc, trade) => {
            const date = format(new Date(trade.entry_time || new Date()), 'yyyy-MM-dd');
            if (!acc[date]) {
              acc[date] = { trades: [], totalPnl: 0, wins: 0 };
            }
            // Use net_profit_usd if available, otherwise profit_usd
            const profit = trade.net_profit_usd !== null ? trade.net_profit_usd : trade.profit_usd;
            const pnl = parseFloat(profit?.toString() || '0');
