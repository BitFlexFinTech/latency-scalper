...
export class RiskManager {
  ...
  private async getDailyLoss(): Promise<number> {
    ...
    // CRITICAL FIX: Get today's closed trades from trade_logs (NEW BOT) - NOT trade_logs (OLD BOT)
    const { data: trades } = await supabase
      .from('trade_logs')
      .select('profit_usd, net_profit_usd, exit_time, status')
    ...
    // Sum up losses (negative PnL) - use net_profit_usd if available, otherwise profit_usd
